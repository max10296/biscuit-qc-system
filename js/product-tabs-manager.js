...
(function () {
    ...
    class ProductTabsManager {
        ...
        collectProductData() {
            ...
            const txt = (id) => (getEl(id)?.value || '').trim();
            ...
            // Required fields
            let product_id = txt('product-id');
            const name = txt('product-name-modal');
            let code = txt('product-batch-code');

            // Auto-generate sensible defaults if missing
            if (!product_id) {
                const slug = (name || 'PRD').toUpperCase().replace(/[^A-Z0-9]+/g, '').slice(0, 6) || 'PRD';
                product_id = `${slug}-${Date.now().toString().slice(-6)}`;
            }
            if (!code) {
                code = (product_id || 'PRD').toString().slice(0, 8);
            }
            ...
            const payload = {
                ...
                name,
                code,
                batch_code: code, // use same short code for batch prefix
                ingredients_type: 'without-cocoa',
                has_cream: false,
                standard_weight: num('product-standard-weight', 185.0),
                shelf_life: parseInt(getEl('product-shelf-life')?.value) || 6,
                cartons_per_pallet: parseInt(getEl('product-cartons-per-pallet')?.value) || 56,
                packs_per_box: parseInt(getEl('product-packs-per-box')?.value) || 6,
                boxes_per_carton: parseInt(getEl('product-boxes-per-carton')?.value) || 14,
                empty_box_weight: num('product-empty-box-weight', 21.0),
                empty_carton_weight: num('product-empty-carton-weight', 680.0),
                aql_level: (getEl('product-aql-level')?.value || '1.5').toString(),
                day_format: (getEl('product-day-format')?.value || 'DD'),
                month_format: (getEl('product-month-format')?.value || 'letter'),
                description: undefined,
                notes: txt('product-notes'),
                customVariables: this._collectCustomVariables(),
                sections: this._collectSections()
            };

            // Basic validation (let unified layer handle user messaging)
        ...
        _collectCustomVariables() {
            ...
            // Try to infer structure: look for rows with inputs having data- attributes or common classnames
            ...
            // Fallback: try generic inputs inside container
        ...
        _collectSections() {
            const out = [];
            // The UI builds child tabs dynamically. Try to look for .child-tabs-content panels
            ...
            panels.forEach((panel, idx) => {
                ...
                const parameters = [];
                // Try to collect parameters within the panel
    ...
    // Provide legacy/global alias expected by other modules
    window.ProductTabsManager = window.ProductTabsManager || {};
    window.ProductTabsManager.init = () => window.productTabsManager.init();
    window.ProductTabsManager.saveProduct = () => window.productTabsManager.saveProduct();
