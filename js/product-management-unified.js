...
saveProductFromForm: function() {
    if (window.ProductTabsManager && typeof window.ProductTabsManager.saveProduct === 'function') {
        ...
        if (!this.validateDynamicRequiredFields()) {
            return;
        }
 ...
 saveProduct: async function(productData) {
    try {
        ...
        validateDynamicRequiredFields: function() {
            const form = document.getElementById('product-form');
            if (!form) {
                return true;
            }
            ...
            for (const field of requiredFields) {
                ...
                if (!value) {
                    ...
                    if (panel && window.productTabsManager && typeof window.productTabsManager.switchToTab === 'function') {
                    ...
                    this.showNotification('Please complete all required fields before saving.', 'error');
                    return false;
                }
            }
