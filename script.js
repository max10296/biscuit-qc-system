document.addEventListener('DOMContentLoaded', () => {
    ...
    function addRecipeTable(container, recipe = null) {
        ...
        <div class="flex justify-between items-center mb-2">
            <div class="flex items-center flex-1">
                ...
                <input type="text" class="input-field recipe-name-input" name="recipe-name" value="${recipe ? recipe.name : ''}" placeholder="Recipe Name (e.g., Dough, Filling)" data-required="true">
            </div>
    ...
    function addIngredientRow(container, ingredient = null) {
        ...
        <input type="text" class="input-field ingredient-name col-span-2" name="ingredient-name" placeholder="Ingredient Name" value="${ingredient ? ingredient.name : ''}" data-required="true">
        <input type="text" class="input-field ingredient-weight" name="ingredient-weight" placeholder="Weight" value="${ingredient ? ingredient.weight : ''}" data-required="true">
        <input type="number" class="input-field ingredient-shelf-life" name="ingredient-shelf-life" placeholder="Shelf Life (m)" value="${ingredient ? ingredient.shelfLife : ''}" data-required="true">
...
`;
    ...
    function openProductModal(product = null) {
        if (!productForm || !productModal) {
            console.error('Product modal structure is missing.');
            return;
        }

        if (window.productTabsManager && typeof window.productTabsManager.init === 'function') {
            window.productTabsManager.init();
            if (typeof window.productTabsManager.reinitializeEventHandlers === 'function') {
                window.productTabsManager.reinitializeEventHandlers();
            }
        }

        const setFieldValue = (id, value, options = {}) => {
            const element = document.getElementById(id);
            if (!element) {
                console.warn(`Product field "${id}" not found in modal.`);
                return null;
            }
        ...
        const getNormalizedId = (item) => {
            if (!item) return '';
            return item.id || item.product_id || item.productId || '';
        };

        editingProductId = product ? getNormalizedId(product) : null;
        productForm.reset();
        ...
        if (product) {
            modalTitle && (modalTitle.textContent = 'Edit Product');

            const productIdValue = getNormalizedId(product);
            setFieldValue('product-id', productIdValue, { disabled: true });
            setFieldValue('product-uuid', productIdValue);
            setFieldValue('product-name-modal', product.name || '');
            setFieldValue('product-standard-weight', product.standardWeight ?? product.standard_weight ?? '');
            setFieldValue('product-shelf-life', product.shelfLife ?? product.shelf_life ?? '');
            setFieldValue('product-cartons-per-pallet', product.cartonsPerPallet ?? product.cartons_per_pallet ?? '');
            setFieldValue('product-packs-per-box', product.packsPerBox ?? product.packs_per_box ?? '');
            setFieldValue('product-boxes-per-carton', product.boxesPerCarton ?? product.boxes_per_carton ?? '');
            setFieldValue('product-empty-box-weight', product.emptyBoxWeight ?? product.empty_box_weight ?? '');
            setFieldValue('product-empty-carton-weight', product.emptyCartonWeight ?? product.empty_carton_weight ?? '');
            setFieldValue('product-aql-level', product.aqlLevel ?? product.aql_level ?? '1.0%');
            setFieldValue('product-batch-code', product.batchCode ?? product.batch_code ?? product.code ?? '');
            setFieldValue('product-doc-code', product.code ?? '');
            setFieldValue('product-day-format', product.dayFormat ?? product.day_format ?? 'DD');
            setFieldValue('product-month-format', product.monthFormat ?? product.month_format ?? 'letter');
            setFieldValue('product-notes', product.notes ?? '');

            if (typeof updateBatchPreview === 'function') {
                updateBatchPreview();
            }

            const normalizedSections = Array.isArray(product.sections)
                ? product.sections
                : product.sections
                    ? Object.keys(product.sections).map(key => ({ id: key, ...product.sections[key] }))
                    : [];

            if (normalizedSections.length && typeof window.addFormSection === 'function') {
                normalizedSections.forEach(sectionData => window.addFormSection(sectionData));
                const firstSection = normalizedSections[0];
                if (firstSection && typeof window.switchChildTab === 'function') {
                    const firstId = firstSection.id || firstSection.section_id;
                    if (firstId) {
                        setTimeout(() => window.switchChildTab(firstId), 50);
                    }
                }
            } else if (typeof window.addFormSection === 'function') {
                window.addFormSection();
            }

            if (Array.isArray(product.customVariables) && typeof window.addCustomVariable === 'function') {
                product.customVariables.forEach(variable => window.addCustomVariable(variable));
            }

            if (recipeConfigContainer && typeof addRecipeTable === 'function') {
            ...
            if (Array.isArray(product.qualityCriteria) && typeof addQualityCriteria === 'function') {
                product.qualityCriteria.forEach(criteria => addQualityCriteria(criteria));
            }
        } else {
            modalTitle && (modalTitle.textContent = 'Add Product');
            setFieldValue('product-id', '', { disabled: false });
            setFieldValue('product-uuid', '');
            setFieldValue('product-name-modal', '');
            setFieldValue('product-doc-code', '');
            setFieldValue('product-batch-code', '');
