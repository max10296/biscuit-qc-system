...
const REQUIRED_PRODUCT_FIELDS = new Set(['product_id', 'name', 'code']);

const prepareProductUpdatePayload = (data = {}) => {
  const sanitized = {};
  const errors = [];
  ...
  Object.entries(data).forEach(([key, value]) => {
    ...
    if (typeof value === 'string') {
      ...
      if (trimmed === '') {
        if (REQUIRED_PRODUCT_FIELDS.has(key)) {
...
// GET /api/products/:id - Get single product with configuration
router.get('/products/:id', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.warn('Falling back to manual product configuration retrieval', {
      productId: id,
      error: error.message
    });
  }
  ...
  try {
    const [customVariablesResult, sectionsResult] = await Promise.all([
      db.query(
        `SELECT * FROM product_custom_variables WHERE product_id = $1 ORDER BY created_at ASC`,
        [id]
      ),
      db.query(
        `SELECT * FROM product_sections WHERE product_id = $1 ORDER BY order_index ASC`,
        [id]
      )
    ]);

    customVariables = customVariablesResult.rows;
    sections = sectionsResult.rows;

    if (sections.length > 0) {
      const sectionIds = sections.map(section => section.id);
      const paramsResult = await db.query(
        `SELECT * FROM product_parameters WHERE section_id = ANY($1::uuid[]) ORDER BY order_index ASC`,
        [sectionIds]
      );
      parameters = paramsResult.rows;
    }
  } catch (fallbackError) {
    logger.warn('Unable to load full product configuration during fallback', {
      productId: id,
...
router.put('/products/:id', asyncHandler(async (req, res) => {
  ...
  const { sanitized, errors } = prepareProductUpdatePayload(updateData);
  ...
  const updated = await db.updateById('products', id, sanitized);
