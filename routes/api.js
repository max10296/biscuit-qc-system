...
router.get('/products/:id', asyncHandler(async (req, res) => {
  ...
  // Try database function first (if exists), else build config manually
  try {
    ...
    // swallow and fallback to manual build
  }

  // Manual fallback: assemble product configuration from tables
  const product = await db.findById('products', id);
  if (!product) {
    return res.status(404).json({ error: 'Product not found' });
  }
  const customVariables = await db.findWhere('product_custom_variables', { product_id: id }, 'name');
  const sections = await db.findWhere('product_sections', { product_id: id }, 'order_index, section_name');
  const configSections = [];
  for (const s of sections) {
    const params = await db.findWhere('product_parameters', { section_id: s.id }, 'order_index, parameter_name');
    configSections.push({ section: s, parameters: params });
  }
  return res.json({ product, customVariables, sections: configSections });
}));
